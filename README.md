...
| Group | Boards |
| --- | --- |
| Arduino | UNO, UNO R4 WiFi, Nano, Mega 2560 |
| ESP32 | DEVKIT V1, WROOM-32U DevKitC V4, C3 Super Mini, C3 OLED 0.42, S3 N16R8, ESP32-CAM |
| ESP8266 | NodeMCU V3, Wemos D1 Mini, ESP-01 |
| BBC micro:bit | V2, V1 |

Each board carries its real pin list, capabilities, current limits, logic level
and the traps specific to it. That data is sent to the model with every request,
and is also used locally to check the answer.

## The three modes

Buttons above the message box, and a default you can set in Settings.

- **Socratic** (default). One question at a time through trigger, input,
  ...
  worked example. Show solution stays locked until the learner has typed a real
  attempt.
...
Press Connect board in the Serial panel. This uses Web Serial, which needs
Chrome, Edge or Opera on a desktop. Firefox and Safari do not support it and the
app says so.

Send to assistant drops the recent serial output straight into the chat, which
...
Inside `index.html` the script is in labelled sections: config, board
illustrations, board profiles, component library, storage, pin validator,
socratic engine, gemini service, then the UI.
...
| "The port is already in use" | Close the MakeCode tab, Arduino IDE, or any other serial monitor. |
